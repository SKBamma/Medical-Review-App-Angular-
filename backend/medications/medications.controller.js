...
export const get_image_by_image_id = async (req, res, next) => {
    ...
    const results = await Medication.findOne({ "image._id": image_id },
        { image: 1, _id: 0 }).lean();
...
export const verify_med_existed = async (req, res, next) => {
    try {
        const { med_name } = req.body;
        const med_existed = await Medication.findOne({ name: med_name });
